...
Every package keeps its source under `src/`, laid out by kind like the shadcn monorepo template and next-forge's packages: React components in `src/components` (the shadcn CLI writes primitives there too, via the `ui` alias in `packages/ui/components.json`), hooks in `src/hooks`, helpers in `src/lib`. The blocks package is the one exception and follows turbo-start-sanity's `sanity-blocks` layout: a block is a vertical folder (`src/<block>/`) holding its schema, GROQ projection, renderer, Markdown serializer, tests and stories, because the block, not the file kind, is the unit of change there; the primitives shared across blocks (rich text, buttons, images, icons, Markdown helpers, hooks) sit flat in `src/internal/`, with test mocks in `src/internal/testing/`. Packages expose concrete modules through `package.json` exports (`@repo/blocks/hero/hero-block`, `@repo/seo/route`); there are no barrel files, so bundlers only load what a route imports.
