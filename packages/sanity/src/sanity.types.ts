...
// Query: *[_type == "page" && site == $site && language == $locale && slug.current == $path][0]{    _id,    _type,    site,    language,    title,    description,    "slug": slug.current,      seoTitle,  seoDescription,  seoNoIndex,  ogTitle,  ogDescription,  "ogImage": select(    defined(seoImage.asset->url) => seoImage,    defined(image.asset->url) => image,    null  ){      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(alt, asset->altText, caption),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  },      "translations": *[_type == "translation.metadata" && references(^._id)][0]    .translations[defined(value)]{      language,      "site": value->site,      "slug": value->slug.current    }  }
