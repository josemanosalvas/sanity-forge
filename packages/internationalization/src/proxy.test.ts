...
test("the site comes from the Host header, not the server URL", () => {
  ...
  expect(
    resolveSite(request("http://localhost:3000/", "localhost:3000"), "brand-b")
      .key
  ).toBe("brand-b");
});
...
test("public paths are rewritten to the internal site + locale route", () => {
  ...
  expect(context).toStrictEqual({ site: "brand-a", locale: "en" });
...
test("an explicit default-locale prefix redirects to the clean URL", () => {
  ...
  expect(response.headers.get("location")).toBe("http://brand-a.example/about");
});
