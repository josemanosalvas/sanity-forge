...
const targetRoot = path.resolve(packageRoot, targetRootArg);
const allowedRoot = path.resolve(packageRoot, "..", "..");
const rel = path.relative(allowedRoot, targetRoot);
...
const syncThumbnails = async () => {
  ...
  await Promise.all(
    existingEntries
      ...
      .map((entry) => rm(path.join(targetRoot, entry.name), { force: true }))
  );
  ...
  await Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        const sourceFile = path.join(sourceRoot, entry.name, "thumbnail.png");
        ...
        const targetFile = path.join(targetRoot, generatedFileName(entry.name));
